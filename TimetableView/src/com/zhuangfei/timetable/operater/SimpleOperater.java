...
public class SimpleOperater extends AbsOperater{
    ...
    protected void initAttr(AttributeSet attrs) {
        ...
        int maxSlideItem = ta.getInteger(R.styleable.TimetableView_max_slide_item, 10);
    ...
    public void newSlideView(LinearLayout slidelayout) {
        ...
        listener.onInit(slidelayout, 0f);
        for (int i = 0; i < mView.maxSlideItem()/2; i++) {
            View view = listener.getView(i, inflater, mView.itemHeight()*2, mView.marTop()*2);
//            view.setBackgroundResource(R.drawable.slide_background);
            slidelayout.addView(view);
//            view.animate().scaleX(1f).scaleY(1f).setInterpolator(new OvershootInterpolator()).start();
        }
    }
    ...
    private View newItemView(final List<Schedule> originData, final List<Schedule> data, final Schedule subject, Schedule pre, int i, int curWeek) {
        ...
        if (isThisWeek) {
            ...
            if(!colorMap.isEmpty()&&colorMap.containsKey(subject.getName())){
                ...
                gd.setColor(mView.colorPool().getColorAutoWithAlpha(subject.getColorRandom(), 0.15f));
                c1=mView.colorPool().getColorAutoWithAlpha(subject.getColorRandom(), 1f);
            }
            ...
            if(!colorMap.isEmpty()&&mView.colorPool().isIgnoreUserlessColor()&&colorMap.containsKey(subject.getName())){
                ...
                gd.setColor(mView.colorPool().getUselessColorWithAlpha(0.3f));
                c1=mView.colorPool().getUselessColorWithAlpha(1f);
            }
...
//        mView.dateAlpha()
        listener.onInit(dateLayout,0.5f );
